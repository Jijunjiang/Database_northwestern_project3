...
public:
    ...
    DiskSystem(const string &filestem, const bool create = false, const SIZE_T offset = 0, const SIZE_T blocks = 0,
               const SIZE_T blocksize = 0, const SIZE_T heads = 0, const SIZE_T blockspertrack = 0,
               const SIZE_T tracks = 0, const double avgseek = 0, const double trackseek = 0, const double rotlat = 0);

    DiskSystem() { throw GenericException(); }

    DiskSystem(const DiskSystem &rhs) { throw GenericException(); }

    DiskSystem &operator=(const DiskSystem &rhs) {
    ...
    ERROR_T Read(const SIZE_T inoffblock, const SIZE_T numblock, vector <Block> &blocks, double &reqtime);

    ERROR_T Read(const SIZE_T inoffblock, Block &blocks, double &reqtime);

    ERROR_T Write(const SIZE_T inoffblock, const SIZE_T numblock, const vector <Block> &blocks, double &reqtime);

    ERROR_T Write(const SIZE_T inoffblock, const Block &blocks, double &reqtime);
    ...
    //
    ERROR_T NotifyAllocateBlocks(const SIZE_T offset, const SIZE_T innumblocks);

    ERROR_T NotifyDeallocateBlocks(const SIZE_T offset, const SIZE_T innumblocks);
