...
protected:
    ...
    ERROR_T AddKeyPtrVal(const SIZE_T node, const KEY_T &key, const VALUE_T &value, const SIZE_T &newNode);

    ERROR_T InsertInternal(const SIZE_T &node, const KEY_T &key, const VALUE_T &value);

    ERROR_T SanityCheckInternal(const SIZE_T &node, const KEY_T &key, const SIZE_T &isLeft) const;

//#######################################################
    ERROR_T AddKeyPtrVal_n(const SIZE_T &parnode,  const KEY_T &splitKey, const VALUE_T &value, const SIZE_T splitNode);
    ERROR_T SplitNodeN(const SIZE_T &node, SIZE_T &newNode, KEY_T &splitKey);
    ERROR_T InsertInternal_n(const SIZE_T &node, const KEY_T &key,const VALUE_T &value);
    SIZE_T NodeIsFull_n(const SIZE_T &node) ;
    ERROR_T Initial(const KEY_T &key, const VALUE_T &value, BTreeNode &root);
    ERROR_T Splitroot(BTreeNode &root);
